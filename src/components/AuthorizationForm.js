...
function AuthorizationForm({ mode, onLogin }) {
    ...
    return (
        <div>
            <div className="block"><h1>Sign in</h1></div>
            {localStorage.getItem("login_error") ?
                <div className="d-flex flex-row justify-items-center">
                    ...
                    <span className="error_message">{localStorage.getItem("login_error")}</span>
                </div>
                : <div></div>}
            <button className="file-link" onClick={goToRegistration}>Sign up</button>
